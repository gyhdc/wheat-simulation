...
class Concat_ATT(nn.Module):
    def __init__(self, channel,dimension=1):
        ...
        self.d = dimension
        self.att = CBAMBlock(channel)

    def forward(self, x):
        return self.att(torch.cat(x, self.d))
    
class Concat_SK_ATT(nn.Module):
    def __init__(self, channel,dimension=1):
        super(Concat_SK_ATT, self).__init__()
        self.d = dimension
...
class Conv_SK_ATT(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_SK_ATT, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.att = SKAttention(c2)
...
class SPPCSPC_SK_ATT(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        super(SPPCSPC_SK_ATT, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
